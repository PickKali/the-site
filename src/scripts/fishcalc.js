...
function calc() {
  ...
  if (o) {
    luck = [
      (1 - (r + e + l + m)) / locations[settings.location][0],
  ...
  for (let count of locations[settings.location]) {
    for (let i = 0; i < count; i++) {
      let square =
        '<div class="square ' + rarities[rarity] + '"><p><br><normal>1/';
      square += (1 / luck[rarity]).toFixed(0);
      square += "</normal><br><albino>1/";
      square += (
        1 /
        (luck[rarity] * (1 / (5000 * ((100 - settings.variant) / 100))))
      ).toFixed(0);
      square += "</albino><br><melan>1/";
      square += (
        1 /
        (luck[rarity] * (1 / (10000 * ((100 - settings.variant) / 100))))
      ).toFixed(0);
      square += "</melan><br><trophy>1/";
      square += (
        1 /
        (luck[rarity] * (1 / (15000 * ((100 - settings.variant) / 100))))
      ).toFixed(0);
      square += "</trophy><br><giga> 1/";
      square += (
        1 /
        ((0.2 + settings.scale * 0.001) * 0.01 * luck[rarity])
      ).toFixed(0);
      square += "</giga></p></div>";
      full += square;
    }
